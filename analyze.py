...
plt.figure()
# mask = (df['Time (s)'] >= hf_start-5) & (df['Time (s)'] <= hf_end+5)
mask = (df['Time (s)'] >= 0) & (df['Time (s)'] <= 209)

plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'LOX Reg Downstream (psi)'], label='LOX Reg Downstream')
# plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'LOX Tank Upstream (psi)'], label='LOX Tank Upstream')
plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'LOX Tank Downstream (psi)'], label='LOX Tank Downstream')
plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'LOX Engine Manifold (psi)'], label='LOX Engine Manifold')
...
plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'Fuel Reg Downstream (psi)'], label='Fuel Reg Downstream')
plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'Fuel Tank Upstream (psi)'], label='Fuel Tank Upstream')
plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'Fuel Tank Downstream (psi)'], label='Fuel Tank Downstream')
plt.plot(df.loc[mask, 'Time (s)']-hf_start, df.loc[mask, 'Fuel Engine Manifold (psi)'], label='Fuel Engine Manifold')
...
for load_df in [load_a, load_b, load_c]:
    #values added to the raw voltages #only for triton hf 1
    # m = 2900.13
    # b= 0.03433
...
# letters = ["A", "B", "C"]
# index = 0
mask = (df['Time (s)'] >= hf_start-1) & (df['Time (s)'] <= hf_end+2)
...
plt.plot(df.loc[mask, 'Time (s)']-148.4, total_thrust[mask], linewidth=2, label='Triton Thrust (raw)', color='gray', alpha=0.5)
...
plt.plot(df.loc[mask, 'Time (s)']-148.4, smoothed_thrust, linewidth=2, label='Triton Thrust (smoothed)', color='blue')

# Read the prometheus file
prom_df = pd.read_csv('promethesus.csv')
prom_mask = (prom_df['Time'] >= 196) & (prom_df['Time'] <= 206)
plt.plot(prom_df.loc[prom_mask, 'Time']+186-hf_start, prom_df.loc[prom_mask, 'Force (lbs)'], label='Prometheus (for scale)', color='red')
...
burn_mask = (df['Time (s)'] >= hf_start) & (df['Time (s)'] <= hf_end)
long_mask = (df['Time (s)'] >= hf_start-5) & (df['Time (s)'] <= hf_end+3)
pre_mask =  (df['Time (s)'] >= hf_start-5) & (df['Time (s)'] <= hf_start-1)
post_mask =  (df['Time (s)'] >= hf_end+1) & (df['Time (s)'] <= hf_end+5)
# Get LOX weight data
...
plt.figure()
plt.plot(long_time - hf_start, lox_weight_smooth, label='LOX Weight', color='blue', alpha=0.25)
plt.plot(long_time - hf_start, fuel_weight_smooth, label='Fuel Weight', color='red', alpha=0.25)
...
# plt.plot(time-hf_start, np.poly1d(lox_fit)(time), color='blue', label='LOX Average Weight')
# plt.plot(time-hf_start, np.poly1d(fuel_fit)(time), color='red', label='Fuel Average Weight')
...
x_fuel_fit = [hf_start, hf_end]
y_fuel_fit = [fuel_weight_pre, fuel_weight_post]
fuel_fit = np.polyfit(x_fuel_fit, y_fuel_fit, 1)
plt.plot([x - hf_start for x in x_fuel_fit], np.poly1d(fuel_fit)(x_fuel_fit), color='red', label='Fuel Weight Trendline')

#repeating the process for lox
x_lox_fit = [hf_start-.25, hf_end]
y_lox_fit = [lox_weight_pre, lox_weight_post]
lox_fit = np.polyfit(x_lox_fit, y_lox_fit, 1)
plt.plot([x - hf_start for x in x_lox_fit], np.poly1d(lox_fit)(x_lox_fit), color='blue', label='LOX Weight Trendline')
...
mdot_ox_sys, mdot_fuel_sys = mdot_from_sys(hf_start, hf_end)
...
plt.figure()
plt.plot(df.loc[burn_mask, 'Time (s)']- hf_start, mdot_fuel_sys, color='gray', alpha=0.5, label='Fuel mdot (raw)')
plt.plot(df.loc[burn_mask, 'Time (s)']- hf_start, mdot_fuel_smooth, color='red', label='Fuel mdot (smoothed)')
plt.plot(df.loc[burn_mask, 'Time (s)']- hf_start, mdot_ox_sys, color='gray', alpha=0.5, label='LOX mdot (raw)')
plt.plot(df.loc[burn_mask, 'Time (s)']- hf_start, mdot_lox_smooth, color='blue', label='LOX mdot (smoothed)')
plt.xlabel('Time (s)')
...
#OF plot
plt.figure()
# Calculate tank-based O/F ratio
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, [mdot_lox_tank/mdot_fuel_tank]*len(df.loc[burn_mask]), '--', color='gray', label='O/F Ratio (from slope prediction)', alpha=0.5)
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, mdot_lox_smooth/mdot_fuel_smooth, label='O/F Ratio')
plt.xlabel('Time (s)')
...
plt.figure()
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, Pc.values, color='gray', alpha=0.5, label='Chamber Pressure from Fuel Sensor (raw)')
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, Pc_ox.values, color='gray', alpha=0.5, label='Chamber Pressure from Ox Sensor (raw)')
...
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, Pc_smooth, color='red', label='Chamber Pressure from fuel sensor (smoothed)')
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, Pc_smooth_ox, color='blue', label='Chamber Pressure from ox sensor(smoothed)')
...
plt.figure()
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, isp, linewidth=2, label='Triton ISP (raw)', color='gray', alpha=0.5)
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, isp_smooth, linewidth=2, label='Triton ISP (smoothed)', color='blue')
...
plt.figure()
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, c_star, color='gray', alpha=0.5, label='C* (raw)')
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, [c_star_theory]*len(df.loc[burn_mask]), '--', color='red', label='C* Theory')

# Apply smoothing
c_star_smooth = pd.Series(c_star).rolling(window=window_size*3, center=True).mean()
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, c_star_smooth, color='blue', label='C* (smoothed)')
...
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, c_star_eff, color='gray', alpha=0.5, label='C* Efficiency (raw)')
plt.plot(df.loc[burn_mask, 'Time (s)']-hf_start, c_star_eff_smooth, color='blue', label='C* Efficiency (smoothed)')
